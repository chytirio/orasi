...
use bridge_core::{BridgeResult, TelemetryBatch, TelemetryRecord, TelemetryData, TelemetryType};
use bridge_core::types::{MetricData, MetricValue, MetricType};
...
async fn main() -> BridgeResult<()> {
    ...
    info!("  Average execution time: {:.2}ms", stats.avg_execution_time_ms);
    ...
    for i in 0..50 {
        let record = TelemetryRecord {
            ...
            data: TelemetryData::Trace(bridge_core::types::TraceData {
                ...
                end_time: Some(Utc::now() - chrono::Duration::minutes(i as i64) + chrono::Duration::milliseconds(100)),
