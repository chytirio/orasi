...
use arrow_array::{
    Array, ArrayRef, Float64Array, Int64Array, RecordBatch, StringArray, TimestampNanosecondArray,
};
...
pub fn telemetry_batch_to_record_batch(batch: &TelemetryBatch) -> BridgeResult<RecordBatch> {
    ...
    for record in &batch.records {
        ...
        let attributes_json =
            serde_json::to_string(&record.attributes).unwrap_or_else(|_| "{}".to_string());
...
pub fn record_batch_to_telemetry_batch(batch: &RecordBatch) -> BridgeResult<TelemetryBatch> {
    ...
    let batch_id = if batch.num_rows() > 0 {
        ...
        Uuid::parse_str(&batch_id_str).map_err(|e| {
    ...
    for row_idx in 0..batch.num_rows() {
        ...
        let attributes: HashMap<String, String> =
            serde_json::from_str(&attributes_json).unwrap_or_else(|_| HashMap::new());
        let tags: HashMap<String, String> =
            serde_json::from_str(&tags_json).unwrap_or_else(|_| HashMap::new());
            ...
            TelemetryType::Metric => {
                if let (Some(name), Some(value)) = (metric_name, metric_value) {
                    ...
                    return Err(bridge_core::BridgeError::serialization(format!(
                        "Missing metric name or value in row {}",
                        row_idx
                    )));
                }
            }
...
pub fn serialize_to_arrow_ipc(batch: &TelemetryBatch) -> BridgeResult<Vec<u8>> {
...

    {
        ...
        writer.finish().map_err(|e| {
            bridge_core::BridgeError::serialization(format!(
                "Failed to finish Arrow IPC writer: {}",
                e
            ))
        })?;
    }
...
pub fn deserialize_from_arrow_ipc(data: &[u8]) -> BridgeResult<TelemetryBatch> {
    let mut reader = StreamReader::try_new(data, None).map_err(|e| {
        bridge_core::BridgeError::serialization(format!("Failed to create Arrow IPC reader: {}", e))
    })?;

    let record_batch = reader
        .next()
        .ok_or_else(|| {
            bridge_core::BridgeError::serialization("No data found in Arrow IPC stream".to_string())
        })?
        .map_err(|e| {
            bridge_core::BridgeError::serialization(format!("Failed to read Arrow IPC data: {}", e))
        })?;
...
fn get_string_value(
    ...
    let column_idx = batch
        ...
        .ok_or_else(|| {
            bridge_core::BridgeError::serialization(format!("Column '{}' not found", column_name))
        })?
        .0;
...
fn get_timestamp_value(
    ...
    let array = column
        ...
        .ok_or_else(|| {
            bridge_core::BridgeError::serialization(format!(
                "Column '{}' is not a timestamp array",
                column_name
            ))
        })?;
...
mod tests {
    ...
    fn test_telemetry_batch_roundtrip() {
        ...
        assert_eq!(
            original_batch.records.len(),
            deserialized_batch.records.len()
        );
    }
}
