...
use arrow_array::{Array, Float64Array, StringArray, TimestampNanosecondArray, RecordBatch, UInt64Array, BooleanArray, Int64Array};
...
impl KafkaConsumer {
    ...
    async fn new(config: &KafkaConfig) -> BridgeResult<Self> {
        ...
        client_config.set("auto.commit.interval.ms", &config.auto_commit_interval_ms.to_string());
        ...
        let consumer: StreamConsumer = client_config
            .create()
            .map_err(|e| bridge_core::BridgeError::network(
        ...
        consumer.subscribe(&[&config.topic])
            .map_err(|e| bridge_core::BridgeError::network(
                format!("Failed to subscribe to Kafka topic {}: {}", config.topic, e)
            ))?;
    ...
    async fn new(_config: &KafkaConfig) -> BridgeResult<Self> {
        Err(bridge_core::BridgeError::configuration(
            "Kafka feature is not enabled. Enable the 'kafka' feature to use Kafka functionality.".to_string()
        ))
    }
    ...
    async fn poll(&mut self) -> BridgeResult<Vec<(Option<Vec<u8>>, Vec<u8>)>> {
        let consumer = self.consumer.as_ref()
            .ok_or_else(|| bridge_core::BridgeError::network("Kafka consumer not initialized".to_string()))?;
        ...
        match tokio::time::timeout(
            ...
            Ok(Err(e)) => {
                return Err(bridge_core::BridgeError::network(
                    format!("Kafka message consumption error: {}", e)
                ));
            }
            Err(_) => {
...
impl KafkaProtocol {
    ...
    async fn start_consuming(&self) -> BridgeResult<()> {
        ...
        tokio::spawn(async move {
            info!("Kafka message consumption loop started for topic: {}", topic);
                    ...
                    Ok(messages) => {
                        for (key, value) in messages {
                            ...
                            let result = Self::process_kafka_message_internal(
                                &config, &message_handler, key, value
                            ).await;
            ...
            consumer.stop();
            info!("Kafka message consumption loop stopped for topic: {}", topic);
        });
...
impl KafkaMessageHandler {
    ...
    async fn decode_json_message(&self, payload: &[u8]) -> BridgeResult<Vec<TelemetryRecord>> {
        ...
        let json_str = std::str::from_utf8(payload)
            .map_err(|e| bridge_core::BridgeError::serialization(format!(
                "Failed to decode payload as UTF-8: {}", e
            )))?;
    ...
    async fn decode_avro_message(&self, payload: &[u8]) -> BridgeResult<Vec<TelemetryRecord>> {
        ...
        use avro_rs::{Reader, Schema, from_avro_datum};
        use serde_json::Value as JsonValue;
    ...
    async fn decode_protobuf_message(&self, payload: &[u8]) -> BridgeResult<Vec<TelemetryRecord>> {
        ...
        if let Ok(request) = opentelemetry_proto::tonic::collector::metrics::v1::ExportMetricsServiceRequest::decode(payload) {
                        ...
                        for data_point in metric.data {
                            match data_point {
                                ...
                                _ => {
                                    ...
                                    info!("Unsupported Kafka protobuf metric type for metric: {}", metric_name);
                                }
                            }
                        }
                    }
        ...
        if let Ok(request) = opentelemetry_proto::tonic::collector::logs::v1::ExportLogsServiceRequest::decode(payload) {
            let mut records = Vec::new();
    ...
    async fn decode_arrow_message(&self, payload: &[u8]) -> BridgeResult<Vec<TelemetryRecord>> {
        ...
        use arrow_array::{RecordBatch, StringArray, Float64Array, Int64Array};
    ...
    async fn process_arrow_batch(&self, batch: &RecordBatch) -> BridgeResult<Vec<TelemetryRecord>> {
        ...
        for row_idx in 0..num_rows {
            ...
            if let Some(message_array) = message_col.and_then(|col| col.as_any().downcast_ref::<StringArray>()) {
                let message = message_array.value(row_idx);
                let level = if let Some(level_array) = level_col.and_then(|col| col.as_any().downcast_ref::<StringArray>()) {
    ...
    fn avro_value_to_json(&self, avro_value: avro_rs::types::Value) -> JsonValue {
        match avro_value {
            ...
            avro_rs::types::Value::Bytes(b) => JsonValue::String(String::from_utf8_lossy(&b).to_string()),
            avro_rs::types::Value::String(s) => JsonValue::String(s),
            avro_rs::types::Value::Array(arr) => {
                let json_array: Vec<JsonValue> = arr.into_iter().map(|v| self.avro_value_to_json(v)).collect();
            ...
            avro_rs::types::Value::Union(union) => {
                ...
                match *union {
                    avro_rs::types::Value::Union(inner) => {
                        self.avro_value_to_json(*inner)
                    }
                    _ => JsonValue::Null,
                }
            }
            avro_rs::types::Value::Fixed(_, bytes) => JsonValue::String(String::from_utf8_lossy(&bytes).to_string()),
