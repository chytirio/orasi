...
use bridge_core::health::{HealthChecker, HealthCheckConfig, HealthStatus as CoreHealthStatus};
...
impl HealthMonitoringIntegration {
    ...
    pub fn new(config: BridgeAPIConfig, metrics: ApiMetrics) -> Self {
        ...
        let bridge_metrics = Arc::new(BridgeMetrics::new(bridge_core::metrics::MetricsConfig::default()));
    ...
    pub async fn init(&self) -> BridgeResult<()> {
        ...
        health.insert("bridge-api".to_string(), ComponentHealthStatus {
        ...
        health.insert("query-engine".to_string(), ComponentHealthStatus {
        ...
        health.insert("schema-registry".to_string(), ComponentHealthStatus {
        ...
        // Ingestion component
        health.insert("ingestion".to_string(), ComponentHealthStatus {
    ...
    pub async fn get_component_statuses(&self) -> BridgeResult<Vec<ComponentStatus>> {
        ...
        let components: Vec<ComponentStatus> = health.values()
    ...
    pub async fn get_system_metrics(&self) -> BridgeResult<SystemMetrics> {
        ...
        let network_bytes_received = self.get_metric_value(&metrics_snapshot, "bridge_records_processed_total") as u64 * 1024; // Estimate 1KB per record
        let network_bytes_sent = self.get_metric_value(&metrics_snapshot, "bridge_batches_processed_total") as u64 * 1024 * 100; // Estimate 100KB per batch
    ...
    pub async fn update_component_health(
        ...
        if let Some(component) = health.get_mut(component_name) {
            ...
            health.insert(component_name.to_string(), ComponentHealthStatus {
    ...
    pub async fn get_component_metrics(&self, component_name: &str) -> BridgeResult<Option<HashMap<String, f64>>> {
        let health = self.component_health.read().await;
    ...
    pub async fn perform_health_checks(&self) -> BridgeResult<()> {
        ...
        for result in health_results {
            if let Some(component) = health.get_mut(&result.component) {
                ...
                component.metrics.insert("health_check_duration_ms".to_string(), result.duration_ms as f64);
            }
        }
    ...
    pub async fn get_health_stats(&self) -> BridgeResult<HashMap<String, serde_json::Value>> {
        ...
        for (name, component) in health.iter() {
            stats.insert(name.clone(), serde_json::json!({
    ...
    async fn get_cpu_usage(&self) -> f64 {
        ...
        let uptime = self.start_time.elapsed().expect("Failed to get elapsed time").as_secs_f64();
    ...
    async fn get_memory_usage(&self) -> BridgeResult<u64> {
        ...
        let uptime_factor = self.start_time.elapsed().expect("Failed to get elapsed time").as_secs() as u64;
    ...
    fn get_metric_value(&self, snapshot: &HashMap<String, bridge_core::metrics::collector::MetricValue>, key: &str) -> f64 {
        snapshot.get(key)
            .and_then(|value| {
                match value {
    ...
    async fn calculate_request_rate(&self, snapshot: &HashMap<String, bridge_core::metrics::collector::MetricValue>) -> f64 {
    ...
    async fn calculate_error_rate(&self, snapshot: &HashMap<String, bridge_core::metrics::collector::MetricValue>) -> f64 {
