...
use bridge_auth::config::{AuthConfig, OAuthConfig, OAuthProviderConfig, OAuthConfigConverter, BridgeApiOAuthConfig};
use bridge_api::error::{ApiError, EnhancedErrorResponse, ErrorContext, ErrorSeverity};
use streaming_processor::{
    StreamingProcessor, StreamingProcessorConfig, SourceConfig, ProcessorConfig, SinkConfig,
    SourceType, ProcessorType, SinkType, ProcessingConfig, StateConfig, MetricsConfig, SecurityConfig,
    AuthType, ConnectionConfig, init_streaming_processor_with_config,
};
...
fn create_streaming_processor_config() -> StreamingProcessorConfig {
    ...
    sources.insert(
        ...
        SourceConfig {
            ...
            config: {
                ...
                config.insert("endpoint_url".to_string(), serde_json::json!("https://httpbin.org/json"));
    ...
    sources.insert(
        ...
        SourceConfig {
            ...
            config: {
                let mut config = HashMap::new();
                config.insert("bootstrap_servers".to_string(), serde_json::json!("localhost:9092"));
                config.insert("topic".to_string(), serde_json::json!("telemetry-data"));
                config.insert("group_id".to_string(), serde_json::json!("streaming-processor"));
                config
            },
            auth: None,
            connection: ConnectionConfig::default(),
    ...
    let processors = vec![
        ...
        ProcessorConfig {
            ...
            config: {
                let mut config = HashMap::new();
                config.insert("filter_expression".to_string(), serde_json::json!("service_name == 'web-service'"));
                config
            },
            order: 1,
        },
        ...
        ProcessorConfig {
            ...
            config: {
                ...
                config.insert("transform_rules".to_string(), serde_json::json!(vec![
                    {"field": "timestamp", "operation": "format", "value": "ISO8601"},
                    {"field": "level", "operation": "uppercase"}
                ]));
                config
            },
            order: 2,
        ...
        ProcessorConfig {
            ...
            config: {
                ...
                config.insert("aggregation_fields".to_string(), serde_json::json!(vec!["count", "sum", "avg"]));
                config
            },
            order: 3,
        },
    ...
    sinks.insert(
        ...
        SinkConfig {
            ...
            config: {
                let mut config = HashMap::new();
                config.insert("endpoint_url".to_string(), serde_json::json!("https://api.example.com/telemetry"));
                config.insert("method".to_string(), serde_json::json!("POST"));
                config.insert("batch_size".to_string(), serde_json::json!(100));
                config
            },
            auth: Some(auth::config::AuthConfig {
                auth_type: auth::config::AuthType::ApiKey,
                credentials: {
    ...
    sinks.insert(
        ...
        SinkConfig {
            ...
            config: {
                let mut config = HashMap::new();
                config.insert("file_path".to_string(), serde_json::json!("/tmp/processed-data.json"));
...
fn demonstrate_oauth_mapping() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("  Authorization URL: {}", bridge_api_oauth_config.authorization_url);
...
fn demonstrate_error_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for error in errors {
        ...
        info!("Error: {} -> Status: {}, Category: {}, Severity: {:?}",
            error.error_code(),
            error.status_code(),
