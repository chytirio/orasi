...
impl IcebergSchema {
    ...
    pub async fn update_schema(&mut self, new_schema: Schema) -> IcebergResult<()> {
        info!("Updating schema from version {} to {}", self.version, self.version + 1);
    ...
    async fn validate_schema_compatibility(old_schema: &Schema, new_schema: &Schema) -> IcebergResult<()> {
                    ...
                    Ok(new_field) => {
                        if new_field.is_nullable() {
                            return Err(IcebergError::schema(format!(
                                "Required field '{}' cannot become nullable in schema evolution", field.name()
                            )));
                        }
                    }
                    Err(_) => {
                        return Err(IcebergError::schema(format!(
                            "Required field '{}' missing in new schema", field.name()
                        )));
                    }
                }
            }
        ...
        match (old_type, new_type) {
            ...
            (DataType::Timestamp(old_unit, _), DataType::Timestamp(new_unit, _)) => {
                match (old_unit, new_unit) {
                    (arrow::datatypes::TimeUnit::Second, arrow::datatypes::TimeUnit::Millisecond) => true,
                    (arrow::datatypes::TimeUnit::Second, arrow::datatypes::TimeUnit::Microsecond) => true,
                    (arrow::datatypes::TimeUnit::Second, arrow::datatypes::TimeUnit::Nanosecond) => true,
                    (arrow::datatypes::TimeUnit::Millisecond, arrow::datatypes::TimeUnit::Microsecond) => true,
                    (arrow::datatypes::TimeUnit::Millisecond, arrow::datatypes::TimeUnit::Nanosecond) => true,
    ...
    pub async fn from_json(json: serde_json::Value) -> IcebergResult<Self> {
        ...
        let fields_json = json["fields"].as_array()
            .ok_or_else(|| IcebergError::schema("Invalid schema JSON: missing fields".to_string()))?;
        
        let mut fields = Vec::new();
        for field_json in fields_json {
            let name = field_json["name"].as_str()
                .ok_or_else(|| IcebergError::schema("Invalid field JSON: missing name".to_string()))?
                .to_string();
            
            let type_str = field_json["type"].as_str()
                .ok_or_else(|| IcebergError::schema("Invalid field JSON: missing type".to_string()))?;
        ...
        let partition_columns = json["partition_columns"].as_array()
    ...
    fn parse_data_type(type_str: &str) -> IcebergResult<DataType> {
        match type_str {
            ...
            "Timestamp(Microsecond, None)" => Ok(DataType::Timestamp(arrow::datatypes::TimeUnit::Microsecond, None)),
            "Timestamp(Millisecond, None)" => Ok(DataType::Timestamp(arrow::datatypes::TimeUnit::Millisecond, None)),
            "Timestamp(Nanosecond, None)" => Ok(DataType::Timestamp(arrow::datatypes::TimeUnit::Nanosecond, None)),
            _ => Err(IcebergError::schema(format!("Unsupported data type: {}", type_str))),
        }
    }
