...
use bridge_core::types::{MetricData, MetricValue, TelemetryData, TelemetryRecord, TelemetryType, LogData, TraceData, LogLevel, SpanKind, StatusCode, MetricType};
...
use opentelemetry_proto::tonic::{
    ...
    metrics::v1::{
        metric::Data as MetricDataProto,
    ...
    trace::v1::{
        Span, ResourceSpans, ScopeSpans,
    },
    resource::v1::Resource,
};
use arrow_array::{Array, Float64Array, StringArray, TimestampNanosecondArray, RecordBatch, UInt64Array, BooleanArray};
...
impl DataValidator {
    ...
    pub fn validate_batch(&self, batch: &mut TelemetryBatch) -> BridgeResult<()> {
        info!("Validating telemetry batch with {} records", batch.records.len());
    ...
    fn validate_timestamp(&self, timestamp: &mut DateTime<Utc>) -> BridgeResult<()> {
        ...
        if *timestamp < min_time || *timestamp > max_time {
            info!("Timestamp out of range, using current time: {:?}", timestamp);
            *timestamp = now;
        }
    ...
    fn validate_uuid(&self, id: &Uuid) -> BridgeResult<()> {
        if id.is_nil() {
            return Err(bridge_core::BridgeError::validation("Invalid UUID: nil UUID".to_string()));
        }
        Ok(())
    }

    /// Validate and sanitize attributes
    fn sanitize_attributes(&self, attributes: &HashMap<String, String>) -> BridgeResult<HashMap<String, String>> {
        if attributes.len() > self.max_attributes {
            return Err(bridge_core::BridgeError::validation(format!(
                "Too many attributes: {} (max: {})", attributes.len(), self.max_attributes
            )));
        }
    ...
    fn validate_metric_data(&self, metric_data: &mut MetricData) -> BridgeResult<()> {
        ...
        if !self.metric_name_regex.is_match(&metric_data.name) {
            return Err(bridge_core::BridgeError::validation(format!(
                "Invalid metric name: {}", metric_data.name
            )));
        }
        ...
        if metric_data.labels.len() > self.max_labels {
            return Err(bridge_core::BridgeError::validation(format!(
                "Too many labels: {} (max: {})", metric_data.labels.len(), self.max_labels
            )));
        }
    ...
    fn validate_metric_value(&self, value: &MetricValue) -> BridgeResult<()> {
        match value {
            MetricValue::Gauge(v) | MetricValue::Counter(v) => {
                if !v.is_finite() {
                    return Err(bridge_core::BridgeError::validation("Invalid metric value: non-finite".to_string()));
                }
                if *v < -1e9 || *v > 1e9 {
                    return Err(bridge_core::BridgeError::validation("Metric value out of reasonable range".to_string()));
                }
            }
            MetricValue::Histogram { buckets: _, sum: _, count: _ } => {
    ...
    fn validate_trace_data(&self, trace_data: &mut TraceData) -> BridgeResult<()> {
        ...
        if trace_data.trace_id.is_empty() {
            return Err(bridge_core::BridgeError::validation("Empty trace ID".to_string()));
        }
        
        // Validate span ID
        if trace_data.span_id.is_empty() {
            return Err(bridge_core::BridgeError::validation("Empty span ID".to_string()));
        }
        ...
        if let Some(ref mut end_time) = trace_data.end_time {
            ...
            if *end_time < trace_data.start_time {
                return Err(bridge_core::BridgeError::validation("End time before start time".to_string()));
            }
        }
    ...
    /// Validate event data
    fn validate_event_data(&self, event_data: &mut bridge_core::types::EventData) -> BridgeResult<()> {
        // Validate event name
        if event_data.name.trim().is_empty() {
            return Err(bridge_core::BridgeError::validation("Empty event name".to_string()));
        }
    ...
    fn validate_metadata(&self, metadata: &HashMap<String, String>) -> BridgeResult<()> {
        if metadata.len() > self.max_attributes {
            return Err(bridge_core::BridgeError::validation(format!(
                "Too many metadata entries: {} (max: {})", metadata.len(), self.max_attributes
            )));
        }
        
        for (key, value) in metadata {
            if key.len() > self.max_string_length || value.len() > self.max_string_length {
                return Err(bridge_core::BridgeError::validation("Metadata key or value too long".to_string()));
            }
        }
    ...
    fn sanitize_string(&self, input: &str) -> BridgeResult<String> {
        if input.len() > self.max_string_length {
            return Err(bridge_core::BridgeError::validation(format!(
                "String too long: {} (max: {})", input.len(), self.max_string_length
            )));
        }
            ...
            ("otlp", "internal") => {
                ...
                let serialized = serde_json::to_vec(&batch)
                    .map_err(|e| bridge_core::BridgeError::serialization(format!(
                        "Failed to serialize TelemetryBatch to JSON: {}", e
                    )))?;
                Ok(serialized)
            }
            ("internal", "otlp") => {
                // Deserialize from internal format using serde_json
                let batch: TelemetryBatch = serde_json::from_slice(data)
                    .map_err(|e| bridge_core::BridgeError::serialization(format!(
                        "Failed to deserialize TelemetryBatch from JSON: {}", e
                    )))?;
...
impl OtlpConverter {
    ...
    pub async fn convert_from_otlp(&self, data: &[u8]) -> BridgeResult<TelemetryBatch> {
        info!("Converting OTLP data of {} bytes to internal format", data.len());
    ...
    async fn convert_otlp_metrics(&self, request: ExportMetricsServiceRequest) -> BridgeResult<TelemetryBatch> {
    ...
    async fn convert_otlp_metric(&self, metric: Metric, resource_attrs: &HashMap<String, String>) -> BridgeResult<Vec<TelemetryRecord>> {
    ...
    async fn convert_otlp_logs(&self, request: ExportLogsServiceRequest) -> BridgeResult<TelemetryBatch> {
        let mut all_records = Vec::new();
    ...
    async fn convert_otlp_log(&self, log: LogRecord, resource_attrs: &HashMap<String, String>) -> BridgeResult<TelemetryRecord> {
    ...
    async fn convert_otlp_traces(&self, request: ExportTraceServiceRequest) -> BridgeResult<TelemetryBatch> {
        let mut all_records = Vec::new();
    ...
    async fn convert_otlp_span(&self, span: Span, resource_attrs: &HashMap<String, String>) -> BridgeResult<TelemetryRecord> {
        ...
        let status = match span.status.as_ref() {
            Some(status) => match status.code() {
                ...
                opentelemetry_proto::tonic::trace::v1::status::StatusCode::Error => StatusCode::Error,
                _ => StatusCode::Unset,
            },
        ...
        Ok(TelemetryRecord {
            ...
            data: TelemetryData::Trace(TraceData {
                ...
                end_time: if span.end_time_unix_nano > 0 {
                    Some(DateTime::from_timestamp_nanos(span.end_time_unix_nano as i64))
                } else {
                    None
                },
    ...
    fn convert_span_events(&self, events: &[bridge_core::types::SpanEvent]) -> Vec<bridge_core::types::SpanEvent> {
        events.iter().map(|event| bridge_core::types::SpanEvent {
    ...
    fn convert_span_links(&self, links: &[bridge_core::types::SpanLink]) -> Vec<bridge_core::types::SpanLink> {
        links.iter().map(|link| bridge_core::types::SpanLink {
    ...
    /// Convert OTLP span events to internal format
    fn convert_otlp_span_events(&self, events: &[opentelemetry_proto::tonic::trace::v1::span::Event]) -> Vec<bridge_core::types::SpanEvent> {
        events.iter().map(|event| bridge_core::types::SpanEvent {
            name: event.name.clone(),
    ...
    /// Convert OTLP span links to internal format
    fn convert_otlp_span_links(&self, links: &[opentelemetry_proto::tonic::trace::v1::span::Link]) -> Vec<bridge_core::types::SpanLink> {
        links.iter().map(|link| bridge_core::types::SpanLink {
    ...
    pub async fn convert_to_otlp(&self, batch: &TelemetryBatch) -> BridgeResult<Vec<u8>> {
        info!("Converting internal format to OTLP for batch with {} records", batch.records.len());
        ...
        if !metrics.is_empty() {
            ...
            request.encode(&mut buf)
                .map_err(|e| bridge_core::BridgeError::serialization(format!(
                    "Failed to encode OTLP metrics: {}", e
                )))?;
            return Ok(buf);
        }
        ...
        if !traces.is_empty() {
            ...
            request.encode(&mut buf)
                .map_err(|e| bridge_core::BridgeError::serialization(format!(
                    "Failed to encode OTLP traces: {}", e
                )))?;
            return Ok(buf);
        }
        ...
        request.encode(&mut buf)
            .map_err(|e| bridge_core::BridgeError::serialization(format!(
                "Failed to encode empty OTLP request: {}", e
            )))?;
        Ok(buf)
    }

    /// Convert internal metric to OTLP metric
    async fn convert_to_otlp_metric(&self, metric_data: &MetricData, record: &TelemetryRecord) -> BridgeResult<Metric> {
        let data_point = NumberDataPoint {
            ...
            value: Some(match metric_data.value {
                ...
                MetricValue::Histogram { buckets: _, sum: _, count: _ } => NumberDataPointValue::AsDouble(0.0),
        ...
        let metric_data_proto = match metric_data.metric_type {
            MetricType::Gauge => MetricDataProto::Gauge(opentelemetry_proto::tonic::metrics::v1::Gauge {
                data_points: vec![data_point],
            }),
            MetricType::Counter => MetricDataProto::Sum(opentelemetry_proto::tonic::metrics::v1::Sum {
                data_points: vec![data_point],
                is_monotonic: true,
                aggregation_temporality: 1, // Cumulative
            }),
            MetricType::Histogram => MetricDataProto::Histogram(opentelemetry_proto::tonic::metrics::v1::Histogram {
                data_points: vec![opentelemetry_proto::tonic::metrics::v1::HistogramDataPoint {
                    ...
                    count: match &metric_data.value {
                        ...
                        MetricValue::Histogram { buckets: _, sum: _, count: _ } => 0,
            ...
            MetricType::Summary => MetricDataProto::Summary(opentelemetry_proto::tonic::metrics::v1::Summary {
    ...
    async fn convert_to_otlp_log(&self, log_data: &LogData, record: &TelemetryRecord) -> BridgeResult<LogRecord> {
    ...
    async fn convert_to_otlp_span(&self, trace_data: &TraceData, record: &TelemetryRecord) -> BridgeResult<Span> {
        ...
        Ok(Span {
            ...
            parent_span_id: trace_data.parent_span_id.clone().map(|id| id.into()).unwrap_or_default(),
    ...
    async fn convert_event_to_otlp_log(&self, event_data: &bridge_core::types::EventData, record: &TelemetryRecord) -> BridgeResult<LogRecord> {
        // Convert event to log since OTLP doesn't have native events
        let severity_number = 9; // INFO level for events
        
        let message = if let Some(data) = &event_data.data {
            format!("Event: {} - {}", event_data.name, serde_json::to_string(data).unwrap_or_default())
        } else {
...
impl ArrowConverter {
    ...
    pub async fn convert_from_arrow(&self, data: &[u8]) -> BridgeResult<TelemetryBatch> {
        info!("Converting Arrow data of {} bytes to internal format", data.len());
        ...
        loop {
            match reader.next() {
                ...
                Some(Err(e)) => return Err(bridge_core::BridgeError::serialization(format!(
                    "Failed to read Arrow record batch: {}", e
                ))),
                None => break,
            }
        }
    ...
    async fn convert_arrow_record_batch(&self, record_batch: RecordBatch) -> BridgeResult<Vec<TelemetryRecord>> {
        let mut records = Vec::new();
        let num_rows = record_batch.num_rows();

        // Extract common columns
        let timestamp_col = record_batch.column_by_name("timestamp")
            .ok_or_else(|| bridge_core::BridgeError::serialization("Missing timestamp column".to_string()))?;
        let record_type_col = record_batch.column_by_name("record_type")
            .ok_or_else(|| bridge_core::BridgeError::serialization("Missing record_type column".to_string()))?;
        let id_col = record_batch.column_by_name("id")
            .ok_or_else(|| bridge_core::BridgeError::serialization("Missing id column".to_string()))?;
        ...
        for row in 0..num_rows {
            ...
            let record = match record_type.as_str() {
                "metric" => self.convert_arrow_metric_row(&record_batch, row, &id, timestamp).await?,
                "log" => self.convert_arrow_log_row(&record_batch, row, &id, timestamp).await?,
                "trace" => self.convert_arrow_trace_row(&record_batch, row, &id, timestamp).await?,
                _ => {
    ...
    async fn convert_arrow_metric_row(&self, record_batch: &RecordBatch, row: usize, id: &str, timestamp: DateTime<Utc>) -> BridgeResult<TelemetryRecord> {
        let name = self.extract_string_opt(record_batch.column_by_name("name"), row)
            .unwrap_or_else(|| "unknown_metric".to_string());
        let value = self.extract_float64(record_batch.column_by_name("value").ok_or_else(|| bridge_core::BridgeError::serialization("Missing value column".to_string()))?, row)?;
        let metric_type = self.extract_string_opt(record_batch.column_by_name("metric_type"), row)
    ...
    async fn convert_arrow_log_row(&self, record_batch: &RecordBatch, row: usize, id: &str, timestamp: DateTime<Utc>) -> BridgeResult<TelemetryRecord> {
        let message = self.extract_string_opt(record_batch.column_by_name("message"), row)
            .unwrap_or_else(|| "".to_string());
        let level = self.extract_string_opt(record_batch.column_by_name("level"), row)
            .unwrap_or_else(|| "info".to_string());
        let severity_number = self.extract_u64_opt(record_batch.column_by_name("severity_number"), row)
            .unwrap_or(9);
    ...
    async fn convert_arrow_trace_row(&self, record_batch: &RecordBatch, row: usize, id: &str, timestamp: DateTime<Utc>) -> BridgeResult<TelemetryRecord> {
        let trace_id = self.extract_string_opt(record_batch.column_by_name("trace_id"), row)
            .unwrap_or_else(|| "".to_string());
        let span_id = self.extract_string_opt(record_batch.column_by_name("span_id"), row)
            .unwrap_or_else(|| "".to_string());
        let parent_span_id = self.extract_string_opt(record_batch.column_by_name("parent_span_id"), row);
        let name = self.extract_string_opt(record_batch.column_by_name("name"), row)
            .unwrap_or_else(|| "unknown_span".to_string());
        let kind = self.extract_string_opt(record_batch.column_by_name("kind"), row)
    ...
    fn extract_timestamp(&self, column: &Arc<dyn Array>, row: usize) -> BridgeResult<DateTime<Utc>> {
    ...
    fn extract_string(&self, column: &Arc<dyn Array>, row: usize) -> BridgeResult<String> {
        ...
        Err(bridge_core::BridgeError::serialization("Failed to extract string value".to_string()))
    }
    ...
    fn extract_float64(&self, column: &Arc<dyn Array>, row: usize) -> BridgeResult<f64> {
        ...
        Err(bridge_core::BridgeError::serialization("Failed to extract float64 value".to_string()))
    }
    ...
    fn extract_attributes(&self, record_batch: &RecordBatch, row: usize, prefix: &str) -> HashMap<String, String> {
    ...
    pub async fn convert_to_arrow(&self, batch: &TelemetryBatch) -> BridgeResult<Vec<u8>> {
        info!("Converting internal format to Arrow for batch with {} records", batch.records.len());
        ...
        let mut buffer = Vec::new();
        {
            let mut writer = StreamWriter::try_new(&mut buffer, &arrow_batches[0].schema())
                .map_err(|e| bridge_core::BridgeError::serialization(format!(
                    "Failed to create Arrow stream writer: {}", e
                )))?;
            
            for batch in arrow_batches {
                writer.write(&batch)
                    .map_err(|e| bridge_core::BridgeError::serialization(format!(
                        "Failed to write Arrow record batch: {}", e
                    )))?;
            }
            
            writer.finish()
                .map_err(|e| bridge_core::BridgeError::serialization(format!(
                    "Failed to finish Arrow stream: {}", e
                )))?;
        }
    ...
    /// Convert metrics to Arrow record batch
    async fn convert_metrics_to_arrow(&self, metrics: &[&TelemetryRecord]) -> BridgeResult<RecordBatch> {
        ...
        for record in metrics {
            ...
            if let TelemetryData::Metric(metric_data) = &record.data {
                ...
                values.push(match &metric_data.value {
                    ...
                    MetricValue::Histogram { buckets: _, sum: _, count: _ } => 0.0,
                ...
                for (k, v) in &metric_data.labels {
                    labels.entry(k.clone()).or_insert_with(Vec::new).push(v.clone());
                }
                for (k, v) in &record.attributes {
                    attributes.entry(k.clone()).or_insert_with(Vec::new).push(v.clone());
                }
            }
        }
        ...
        // Add label arrays
        for key in labels.keys() {
            let values: Vec<Option<String>> = (0..metrics.len()).map(|i| {
        ...
        for key in attributes.keys() {
            let values: Vec<Option<String>> = (0..metrics.len()).map(|i| {
                metrics[i].attributes.get(key).cloned()
            }).collect();
            arrays.push(Arc::new(StringArray::from(values)));
        }
        ...
        RecordBatch::try_new(schema, arrays)
            .map_err(|e| bridge_core::BridgeError::serialization(format!(
                "Failed to create Arrow metrics record batch: {}", e
            )))
    }
    ...
    async fn convert_logs_to_arrow(&self, logs: &[&TelemetryRecord]) -> BridgeResult<RecordBatch> {
        ...
        // Add attribute arrays
        for key in attributes.keys() {
            let values: Vec<Option<String>> = (0..logs.len()).map(|i| {
        ...
        RecordBatch::try_new(schema, arrays)
            .map_err(|e| bridge_core::BridgeError::serialization(format!(
                "Failed to create Arrow logs record batch: {}", e
            )))
    }

    /// Convert traces to Arrow record batch
    async fn convert_traces_to_arrow(&self, traces: &[&TelemetryRecord]) -> BridgeResult<RecordBatch> {
        let mut timestamps = Vec::new();
        let mut ids = Vec::new();
        ...
        // Add attribute arrays
        for key in attributes.keys() {
            let values: Vec<Option<String>> = (0..traces.len()).map(|i| {
        ...
        RecordBatch::try_new(schema, arrays)
            .map_err(|e| bridge_core::BridgeError::serialization(format!(
                "Failed to create Arrow traces record batch: {}", e
            )))
    }
...
impl JsonConverter {
    ...
    pub async fn convert_to_json(&self, batch: &TelemetryBatch) -> BridgeResult<Vec<u8>> {
        ...
        let json_data = serde_json::to_vec_pretty(batch)
            .map_err(|e| bridge_core::BridgeError::serialization(format!(
                "Failed to serialize TelemetryBatch to JSON: {}", e
            )))?;
...
mod tests {
        ...
        let records = vec![
            ...
            TelemetryRecord {
                ...
                data: TelemetryData::Log(LogData {
                    ...
                    attributes: HashMap::from([
                        ("log_attr".to_string(), "log_value".to_string()),
                    ]),
    ...
    async fn test_otlp_converter_creation() {
        ...
        assert_eq!(converter.supported_source_formats(), vec!["otlp", "internal"]);
        assert_eq!(converter.supported_target_formats(), vec!["otlp", "internal"]);
    }

    #[tokio::test]
    async fn test_arrow_converter_creation() {
        let converter = ArrowConverter::new();
        assert_eq!(converter.supported_source_formats(), vec!["arrow", "internal"]);
        assert_eq!(converter.supported_target_formats(), vec!["arrow", "internal"]);
    }

    #[tokio::test]
    async fn test_json_converter_creation() {
        let converter = JsonConverter::new();
        assert_eq!(converter.supported_source_formats(), vec!["json", "internal"]);
        assert_eq!(converter.supported_target_formats(), vec!["json", "internal"]);
    }
    ...
    async fn test_otlp_converter_internal_roundtrip() {
        ...
        let converted_batch = converter.convert_from_internal(&internal_data).await.unwrap();
        assert_eq!(converted_batch.records.len(), batch.records.len());
        assert_eq!(converted_batch.source, batch.source);
    }
