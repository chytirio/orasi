...
use crate::{
    config::GatewayConfig, error::GatewayError, health::HealthChecker, load_balancer::LoadBalancer,
    metrics::MetricsCollector, proxy::Proxy, rate_limiter::RateLimiter, routing::Router,
    service_discovery::ServiceDiscovery, types::*,
};
use std::sync::Arc;
...
impl GatewayState {
    ...
    fn get_gateway_info(&self) -> GatewayInfo {
        self.gateway_info.clone()
    }

    fn set_status(&mut self, status: GatewayStatus) {
    ...
    fn update_services(&mut self, services: std::collections::HashMap<String, ServiceInfo>) {
        self.services = services;
    }

    fn get_services(&self) -> std::collections::HashMap<String, ServiceInfo> {
        self.services.clone()
    }

    fn update_metrics(&mut self, metrics: GatewayMetrics) {
        self.metrics = metrics;
    }

    fn get_metrics(&self) -> GatewayMetrics {
        self.metrics.clone()
    }
}
