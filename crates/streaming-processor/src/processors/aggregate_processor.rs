...
use bridge_core::{
    ...
    types::{TelemetryRecord, TelemetryData, TelemetryType, MetricValue, MetricData},
    ...
    async fn validate(&self) -> BridgeResult<()> {
        for rule in &self.aggregation_rules {
            ...
            if rule.aggregation_functions.is_empty() {
                return Err(bridge_core::BridgeError::configuration(
                    format!("Aggregation rule '{}' must have at least one function", rule.name)
                ));
            }
        }
...
impl AggregateProcessor {
    ...
    async fn process_data_stream(&self, input: DataStream) -> BridgeResult<DataStream> {
        ...
        info!("Processing data stream with aggregations: {}", input.stream_id);
        ...
        output_metadata.insert("aggregation_rules_applied".to_string(), self.config.aggregation_rules.len().to_string());
        output_metadata.insert("input_records_count".to_string(), records.len().to_string());
        output_metadata.insert("output_records_count".to_string(), aggregated_data.len().to_string());
        ...
        info!("Data stream aggregated in {}ms ({} -> {} records)", 
              processing_time, records.len(), aggregated_data.len());
    ...
    async fn apply_aggregation_rules(&self, records: &[serde_json::Value]) -> BridgeResult<Vec<serde_json::Value>> {
    ...
    async fn apply_single_rule(&self, rule: &AggregationRule, records: &[serde_json::Value]) -> BridgeResult<Vec<serde_json::Value>> {
        ...
        for (group_key, group_records) in groups {
            ...
            for function in &rule.aggregation_functions {
                let aggregated_value = self.apply_aggregation_function(function, &group_records).await?;
                aggregated_record.insert(function.target_field.clone(), aggregated_value);
            }
            
            // Add metadata
            aggregated_record.insert("_group_key".to_string(), serde_json::Value::String(group_key));
            aggregated_record.insert("_record_count".to_string(), serde_json::Value::Number(serde_json::Number::from(group_records.len())));
            aggregated_record.insert("_aggregated_at".to_string(), serde_json::Value::String(Utc::now().to_rfc3339()));
            aggregated_record.insert("_rule_name".to_string(), serde_json::Value::String(rule.name.clone()));
    ...
    async fn apply_aggregation_function(&self, function: &AggregationFunction, records: &[&serde_json::Value]) -> BridgeResult<serde_json::Value> {
        let values: Vec<f64> = records
            .iter()
            .filter_map(|record| {
                record.get(&function.source_field)
                    .and_then(|v| {
        ...
        let result = match function.function_type {
            AggregationFunctionType::Count => serde_json::Value::Number(serde_json::Number::from(records.len())),
            ...
            AggregationFunctionType::First => {
                if let Some(first_record) = records.first() {
                    first_record.get(&function.source_field).cloned().unwrap_or(serde_json::Value::Null)
                } else {
            ...
            AggregationFunctionType::Last => {
                if let Some(last_record) = records.last() {
                    last_record.get(&function.source_field).cloned().unwrap_or(serde_json::Value::Null)
                } else {
                    serde_json::Value::Null
                }
            }
    ...
    async fn serialize_aggregated_data(&self, aggregated_data: &[serde_json::Value]) -> BridgeResult<Vec<u8>> {
        if aggregated_data.len() == 1 {
            // Single record - serialize as JSON object
            serde_json::to_vec(&aggregated_data[0])
                .map_err(|e| bridge_core::BridgeError::serialization(
                    format!("Failed to serialize aggregated data: {}", e)
                ))
        } else {
            // Multiple records - serialize as JSON array
            serde_json::to_vec(aggregated_data)
                .map_err(|e| bridge_core::BridgeError::serialization(
                    format!("Failed to serialize aggregated data: {}", e)
                ))
        }
