...
impl HttpSource {
    ...
    async fn poll_endpoint(&mut self) -> BridgeResult<()> {
        ...
        let response = request.send().await.map_err(|e| {
            bridge_core::BridgeError::internal(format!("HTTP request failed: {}", e))
        })?;
        ...
        if !response.status().is_success() {
            return Err(bridge_core::BridgeError::internal(format!(
        ...
        let response_data = response.bytes().await.map_err(|e| {
            bridge_core::BridgeError::internal(format!("Failed to read response body: {}", e))
        })?;
