...
fn test_serialization_with_metadata() {
    ...
    assert_eq!(
        batch.metadata.get("key1"),
        deserialized.metadata.get("key1")
    );
    assert_eq!(
        batch.metadata.get("key2"),
...
fn test_serialization_with_attributes() {
    ...
    batch.records[0]
        .attributes
        .insert("attr1".to_string(), "value1".to_string());
    batch.records[0]
        .attributes
        .insert("attr2".to_string(), "value2".to_string());

    let serialized = serialize_to_arrow_ipc(&batch).unwrap();
    let deserialized = deserialize_from_arrow_ipc(&serialized).unwrap();

    assert_eq!(
        batch.records[0].attributes.len(),
        deserialized.records[0].attributes.len()
    );
    assert_eq!(
        batch.records[0].attributes.get("attr1"),
        deserialized.records[0].attributes.get("attr1")
    );
    assert_eq!(
        batch.records[0].attributes.get("attr2"),
        deserialized.records[0].attributes.get("attr2")
    );
}
...
fn test_serialization_with_tags() {
    ...
    batch.records[0]
        .tags
        .insert("tag1".to_string(), "value1".to_string());
    batch.records[0]
        .tags
        .insert("tag2".to_string(), "value2".to_string());

    let serialized = serialize_to_arrow_ipc(&batch).unwrap();
    let deserialized = deserialize_from_arrow_ipc(&serialized).unwrap();

    assert_eq!(
        batch.records[0].tags.len(),
        deserialized.records[0].tags.len()
    );
    assert_eq!(
        batch.records[0].tags.get("tag1"),
        deserialized.records[0].tags.get("tag1")
    );
    assert_eq!(
        batch.records[0].tags.get("tag2"),
        deserialized.records[0].tags.get("tag2")
    );
}
...
fn test_serialization_with_metric_labels() {
    ...
    {
        assert_eq!(
            original_metric.labels.len(),
            deserialized_metric.labels.len()
        );
        assert_eq!(
            original_metric.labels.get("label1"),
            deserialized_metric.labels.get("label1")
        );
        assert_eq!(
            original_metric.labels.get("label2"),
            deserialized_metric.labels.get("label2")
        );
    }
}
...
async fn test_concurrent_serialization() {
    ...
    for _ in 0..5 {
        ...
        let handle = task::spawn(async move { serialize_to_arrow_ipc(&batch) });
        handles.push(handle);
    }
...
fn test_serialization_with_special_characters() {
    ...
    batch.records[0].attributes.insert(
    ...
    assert_eq!(
        batch.records[0].attributes.get("special_key"),
...
fn test_serialization_with_unicode() {
    ...
    assert_eq!(batch.source, deserialized.source);
    assert_eq!(
        batch.records[0].attributes.get("unicode_key"),
