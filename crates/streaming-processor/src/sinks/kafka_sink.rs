...
impl KafkaProducer {
    ...
    async fn send_message(&self, key: Option<Vec<u8>>, value: Vec<u8>) -> BridgeResult<()> {
        ...
        info!("Mock: Sending message to Kafka topic {} with key {} ({} bytes)", 
              self.topic, key_str, value.len());
        
        // Simulate network delay
        tokio::time::sleep(tokio::time::Duration::from_millis(10)).await;
        
        info!("Mock: Message sent to Kafka topic {} partition 0 offset 123", self.topic);
...
impl KafkaSink {
    ...
    async fn init_producer(&mut self) -> BridgeResult<()> {
        info!("Initializing Kafka producer for topic: {}", self.config.topic);
    ...
    async fn send_batch(&self, batch: TelemetryBatch) -> BridgeResult<()> {
        ...
        let serialized_data = match self.config.serialization_format {
            ...
            KafkaSerializationFormat::Json => {
                self.serialize_to_json(&batch).await?
            }
            KafkaSerializationFormat::Avro => {
                self.serialize_to_avro(&batch).await?
            }
            KafkaSerializationFormat::Protobuf => {
                self.serialize_to_protobuf(&batch).await?
            }
            KafkaSerializationFormat::Parquet => {
                self.serialize_to_parquet(&batch).await?
            }
        };
        ...
        info!("Batch sent to Kafka in {}ms ({} bytes)", send_time, data_len);
        Ok(())
    }
