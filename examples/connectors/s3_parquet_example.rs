...
//! S3/Parquet connector example
//! 
//! This example demonstrates how to use the S3/Parquet connector
//! to export telemetry data to S3 storage in Parquet format.
...
use s3_parquet::S3ParquetExporter;
use s3_parquet::config::S3ParquetConfig;
...
async fn main() -> BridgeResult<()> {
    ...
    info!("Starting S3/Parquet connector example");

    // Create S3/Parquet configuration
    let s3_parquet_config = S3ParquetConfig {
        bucket_name: "opentelemetry-data".to_string(),
    ...
    info!("S3/Parquet configuration: bucket={}, prefix={}, region={}",
          s3_parquet_config.bucket_name, s3_parquet_config.prefix, s3_parquet_config.region);

    // Create S3/Parquet exporter
    let mut s3_parquet_exporter = S3ParquetExporter::new(s3_parquet_config);
    
    // Initialize the exporter
    match s3_parquet_exporter.initialize().await {
        Ok(()) => info!("S3/Parquet exporter initialized successfully"),
        Err(e) => {
            error!("Failed to initialize S3/Parquet exporter: {}", e);
            return Err(bridge_core::error::BridgeError::exporter(format!("Failed to initialize S3/Parquet exporter: {}", e)));
        }
    }
    ...
    let mut pipeline = TelemetryIngestionPipeline::new(
        Box::new(receiver),
        Box::new(s3_parquet_exporter),
    );

    info!("Starting telemetry ingestion pipeline with S3/Parquet exporter");
    ...
    info!("S3/Parquet connector example completed successfully");
    Ok(())
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_s3_parquet_example_components() {
        // Test S3/Parquet configuration
