...
use streaming_processor::sources::{
    KafkaSource, HttpSource, FileSource,
    StreamSource, SourceConfig, SourceStats,
};
...
async fn test_kafka_source_config_validation() {
    ...
    let mut invalid_config = KafkaSourceConfig::new(
        vec![],
        "test-topic".to_string(),
        "test-group".to_string()
    );
...
async fn test_kafka_source_lifecycle() {
    let config = KafkaSourceConfig::new(vec!["localhost:9092".to_string()], "test-topic".to_string(), "test-group".to_string());
...
async fn test_file_source_creation() {
    let config = FileSourceConfig::new("test.json".to_string(), streaming_processor::sources::file_source::FileFormat::Json);
...
async fn test_file_source_config_validation() {
    ...
    let mut invalid_config = FileSourceConfig::new("test.json".to_string(), streaming_processor::sources::file_source::FileFormat::Json);
