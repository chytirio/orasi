...
impl ExecutionEngine {
    ...
    pub async fn execute_query(&self, executor_name: &str, query: ParsedQuery) -> BridgeResult<QueryResult> {
        if let Some(executor) = self.get_executor(executor_name) {
            executor.execute(query).await
        } else {
            Err(bridge_core::BridgeError::configuration(
                format!("Executor not found: {}", executor_name)
            ))
        }
    }
    ...
    pub async fn create_executor(
        ...
        match config.name() {
            ...
            _ => Err(bridge_core::BridgeError::configuration(
                format!("Unsupported executor: {}", config.name())
            ))
        }
    }
}
