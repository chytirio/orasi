...
impl HudiWriter {
    ...
    pub async fn new(config: HudiConfig) -> HudiResult<Self> {
        info!(
            "Creating Apache Hudi writer for table: {}",
            config.table_name()
        );
    ...
    fn generate_write_metadata(
        ...
        if duration_ms > 0 {
            let records_per_second =
                (record_count as f64 / (duration_ms as f64 / 1000.0)).round() as u64;
            metadata.insert(
...
impl LakehouseWriter for HudiWriter {
    async fn write_metrics(&self, batch: MetricsBatch) -> BridgeResult<WriteResult> {
        debug!(
            "Writing metrics batch to Apache Hudi: {} records",
            batch.metrics.len()
        );
        ...
        let (duration_ms, failed_records, errors) = self
            .simulate_write_operation(batch.metrics.len(), "metrics")
            .await;
        ...
        info!(
            "Successfully wrote {} metrics records to Apache Hudi in {}ms",
            records_written, duration_ms
        );
    ...
    async fn write_traces(&self, batch: TracesBatch) -> BridgeResult<WriteResult> {
        debug!(
            "Writing traces batch to Apache Hudi: {} records",
            batch.traces.len()
        );
        ...
        let (duration_ms, failed_records, errors) = self
            .simulate_write_operation(batch.traces.len(), "traces")
            .await;
        ...
        info!(
            "Successfully wrote {} trace records to Apache Hudi in {}ms",
            records_written, duration_ms
        );
    ...
    async fn write_logs(&self, batch: LogsBatch) -> BridgeResult<WriteResult> {
        debug!(
            "Writing logs batch to Apache Hudi: {} records",
            batch.logs.len()
        );
        ...
        let (duration_ms, failed_records, errors) = self
            .simulate_write_operation(batch.logs.len(), "logs")
            .await;
        ...
        info!(
            "Successfully wrote {} log records to Apache Hudi in {}ms",
            records_written, duration_ms
        );
    ...
    async fn write_batch(&self, batch: TelemetryBatch) -> BridgeResult<WriteResult> {
        debug!(
            "Writing telemetry batch to Apache Hudi: {} records",
            batch.records.len()
        );
        ...
        let (duration_ms, failed_records, errors) = self
            .simulate_write_operation(batch.records.len(), "telemetry")
            .await;
        ...
        info!(
            "Successfully wrote {} telemetry records to Apache Hudi in {}ms",
            records_written, duration_ms
        );
    ...
    async fn flush(&self) -> BridgeResult<()> {
        ...
        info!(
            "Apache Hudi writer flushed successfully in {}ms",
            duration_ms
        );
        Ok(())
    }
    ...
    async fn close(&self) -> BridgeResult<()> {
        ...
        info!(
            "Apache Hudi writer closed successfully in {}ms",
            duration_ms
        );
        Ok(())
    }
