...
async fn test_query_engine_integration(
    ...
    println!("   ❤️  Query Engine Health Check: {}", if is_healthy { "Healthy" } else { "Unhealthy" });
    
    // Test getting stats
    let stats = query_engine.get_stats().await?;
    println!("   📊 Query Engine Stats: {} executors registered", stats.len());
...
async fn test_health_monitoring_integration(
    ...
    // Test bridge status
    let bridge_status = health_monitoring.get_bridge_status().await?;
    println!("   🏗️  Bridge Status: {:?}", bridge_status);
    
    // Test component statuses
    let component_statuses = health_monitoring.get_component_statuses().await?;
    ...
    println!("      - CPU Usage: {:.1}%", system_metrics.cpu_usage_percent);
    println!("      - Memory Usage: {:.1} MB", system_metrics.memory_usage_bytes as f64 / 1024.0 / 1024.0);
    println!("      - Active Connections: {}", system_metrics.active_connections);
    
    // Test component health updates
    health_monitoring.update_component_health(
        "test-component",
        ComponentState::Running,
        None,
        Some(HashMap::from([
            ("test_metric".to_string(), 42.0),
        ])),
    ).await?;
...
async fn test_service_integration(
    ...
    let query_result = query_service.execute_telemetry_query(&telemetry_query).await?;
    println!("   🔍 Query executed successfully: {:?}", query_result.status);
    ...
    println!("   📊 Status retrieved successfully: {:?}", status_result.status);
