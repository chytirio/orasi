...
use bridge_core::traits::{ConnectorStats, LakehouseConnector, LakehouseReader, LakehouseWriter};
use chrono::Utc;
use std::sync::Arc;
...
impl IcebergConnector {
    ...
    pub async fn initialize(&mut self) -> IcebergResult<()> {
        info!(
            "Initializing Apache Iceberg connector for table: {}",
        ...
        let writer = Arc::new(
            IcebergWriter::new(self.config.clone(), catalog.clone(), schema.clone()).await?,
        );
        let reader = Arc::new(
            IcebergReader::new(self.config.clone(), catalog.clone(), schema.clone()).await?,
        );
    ...
    async fn ensure_table_exists(&self) -> IcebergResult<()> {
        debug!(
            "Ensuring Apache Iceberg table exists: {}",
            self.config.table.table_name
        );
...
impl LakehouseConnector for IcebergConnector {
    ...
    async fn connect(config: Self::Config) -> BridgeResult<Self> {
        ...
        connector.initialize().await.map_err(|e| {
            bridge_core::error::BridgeError::lakehouse_with_source(
                "Failed to initialize Apache Iceberg connector",
                e,
            )
        })?;
        Ok(connector)
    ...
    async fn writer(&self) -> BridgeResult<Self::WriteHandle> {
        if let Some(writer) = &self.writer {
            ...
            Err(bridge_core::error::BridgeError::lakehouse(
