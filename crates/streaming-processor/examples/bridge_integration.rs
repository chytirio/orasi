...
use bridge_core::traits::{QueryResultStatus, TelemetryQueryResult};
use bridge_core::{types::TelemetryQueryType, BridgeResult, TelemetryQuery};
use chrono::Utc;
...
impl BridgeIntegrationService {
    ...
    pub async fn execute_streaming_query(
        ...
        let http_source_config = HttpSourceConfig {
            ...
            endpoint_url: format!(
                "{}{}",
                self.config.bridge_api_url, self.config.query_endpoint
            ),
        ...
        let http_sink_config = HttpSinkConfig {
            ...
            endpoint_url: format!(
                "{}{}",
                self.config.bridge_api_url, self.config.results_endpoint
            ),
            method: "POST".to_string(),
            headers: {
                let mut headers = HashMap::new();
        ...
        info!(
            "Completed streaming query: {} batches processed",
            batch_count
        );
        Ok(results)
    }
    ...
    async fn create_streaming_config(
        ...
        config.name = format!("bridge_processor_{}", query.id);
        config.version = "1.0.0".to_string();

        // Configure processing settings
        config.processing.batch_size = self.config.batch_size;
        config.processing.buffer_size = 1000;
        config.processing.num_workers = 4;

        // Configure metrics
        config.metrics.enable_metrics = true;
        config.metrics.collection_interval = std::time::Duration::from_secs(30);

        // Configure state management
        config.state.store_type = streaming_processor::config::StateStoreType::InMemory;
        ...
        for filter in &query.filters {
            if filter.field == "type" || filter.field == "record_type" {
                return Ok(match &filter.value {
                    ...
                    bridge_core::types::FilterValue::Number(n) => n.to_string(),
                    bridge_core::types::FilterValue::Boolean(b) => b.to_string(),
                    bridge_core::types::FilterValue::Array(arr) => arr
                        .iter()
                        .map(|v| match v {
                            bridge_core::types::FilterValue::String(s) => s.clone(),
                            bridge_core::types::FilterValue::Number(n) => n.to_string(),
    ...
    fn create_mock_metrics_data(&self) -> Vec<bridge_core::types::TelemetryRecord> {
        vec![bridge_core::types::TelemetryRecord::new(
    ...
    fn create_mock_generic_data(&self) -> Vec<bridge_core::types::TelemetryRecord> {
        vec![bridge_core::types::TelemetryRecord::new(
            ...
            bridge_core::types::TelemetryData::Metric(bridge_core::types::MetricData {
                ...
                labels: HashMap::from([("source".to_string(), "bridge_integration".to_string())]),
                timestamp: Utc::now(),
            }),
        )]
    }
...
async fn main() -> BridgeResult<()> {
    ...
    let query = TelemetryQuery {
        id: Uuid::new_v4(),
        timestamp: Utc::now(),
        query_type: TelemetryQueryType::Metrics,
        time_range: bridge_core::types::TimeRange::last_hours(1),
        filters: vec![bridge_core::types::Filter::equals(
