...
use datafusion::arrow::array::{StringArray, Int64Array, Float64Array, BooleanArray, TimestampNanosecondArray, Array};
...
use crate::parsers::{ParsedQuery, QueryAst, AstNode, NodeType};
use super::{QueryExecutor, QueryResult, QueryRow, QueryValue, ExecutionStatus, ExecutorStats};
...
impl DataFusionExecutor {
    ...
    pub fn new(config: DataFusionConfig) -> Self {
        ...
        if let Some(_max_memory) = config.max_memory_bytes {
            ctx_config = ctx_config.with_target_partitions(1)
    ...
    async fn telemetry_batch_to_record_batch(&self, batch: &TelemetryBatch) -> BridgeResult<RecordBatch> {
            ...
            match &record.data {
                TelemetryData::Metric(metric) => {
                    match &metric.value {
                        ...
                        bridge_core::types::MetricValue::Histogram { sum, .. } => values.push(format!("{}", sum)),
                        bridge_core::types::MetricValue::Summary { sum, .. } => values.push(format!("{}", sum)),
                    }
                }
        ...
        let record_batch = RecordBatch::try_new(
            ...
            vec![
                ...
                Arc::new(StringArray::from(vec![batch.id.to_string(); batch.records.len()])),
            ],
        ).map_err(|e| bridge_core::BridgeError::query(format!("Failed to create record batch: {}", e)))?;
    ...
    async fn register_telemetry_batch(&self, batch: &TelemetryBatch) -> BridgeResult<()> {
        ...
        self.ctx.register_batch(&table_name, record_batch)
            .map_err(|e| bridge_core::BridgeError::query(format!("Failed to register batch: {}", e)))?;
        
        if self.config.debug_logging {
            info!("Registered telemetry batch {} as table '{}' with {} records", 
                  batch.id, table_name, batch.records.len());
        }
    ...
    fn record_batch_to_query_result(&self, batch: RecordBatch, query_id: Uuid) -> BridgeResult<QueryResult> {
...
impl QueryExecutor for DataFusionExecutor {
    ...
    async fn execute(&self, query: ParsedQuery) -> BridgeResult<QueryResult> {
        ...
        let df = self.ctx.sql(&query.query_text).await
            .map_err(|e| bridge_core::BridgeError::query(format!("SQL parsing error: {}", e)))?;
        let result_batch = df.collect().await
            .map_err(|e| bridge_core::BridgeError::query(format!("Query execution error: {}", e)))?;
