        ...
        for rule in &self.filter_rules {
            if let FilterOperator::Regex = rule.operator {
                if let Err(e) = Regex::new(&rule.value) {
                                    return Err(bridge_core::BridgeError::configuration(
                    format!("Invalid regex pattern in rule '{}': {}", rule.name, e)
                ));
                }
            }
        }
...
impl FilterProcessor {
    ...
    async fn process_data_stream(&self, input: DataStream) -> BridgeResult<DataStream> {
        ...
        // Apply filter rules
        let filtered_records: Vec<TelemetryRecord> = batch.records
            .into_iter()
        ...
        let filtered_data = serde_json::to_vec(&filtered_batch)
            .map_err(|e| bridge_core::BridgeError::internal(
                format!("Failed to serialize filtered batch: {}", e)
            ))?;
        ...
        output_metadata.insert("filter_rules_applied".to_string(), self.config.filter_rules.len().to_string());
