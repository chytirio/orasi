...
impl FileReader {
    ...
    async fn parse_json_lines(&self, data: &[u8]) -> BridgeResult<Vec<TelemetryRecord>> {
        let json_str = std::str::from_utf8(data).map_err(|e| {
            bridge_core::BridgeError::configuration(
                format!("Invalid UTF-8 in JSON Lines data: {}", e)
            )
        })?;
        ...
        if data.len() >= 4 {
            ...
            let is_parquet_like = data.len() > 100 && 
                (data.starts_with(b"PAR1") || 
                 data.iter().any(|&b| b.is_ascii_alphanumeric()));
    ...
    async fn parse_avro(&self, data: &[u8]) -> BridgeResult<Vec<TelemetryRecord>> {
        use avro_rs::{Schema, from_avro_datum, types::Value};
        use std::io::Cursor;
                ...
                while let Some(batch_result) = reader.next() {
                    match batch_result {
                        Ok(batch) => {
                            ...
                            let batch_records = self.arrow_batch_to_telemetry_records(&batch).await?;
    ...
    async fn json_to_telemetry_records(&self, json_value: serde_json::Value) -> BridgeResult<Vec<TelemetryRecord>> {
        let mut records = Vec::new();
    ...
    fn json_value_to_telemetry_record(&self, json_value: serde_json::Value) -> BridgeResult<TelemetryRecord> {
        ...
        let record_type = json_value.get("type")
            .and_then(|t| t.as_str())
            .map(|s| match s {
        ...
        let attributes = json_value.get("attributes")
    ...
    fn csv_line_to_telemetry_record(&self, line: &str) -> BridgeResult<TelemetryRecord> {
        ...
        if fields.len() < 3 {
            return Err(bridge_core::BridgeError::configuration(
                format!("CSV line must have at least 3 fields: {}", line)
            ));
        }
    ...
    /// Convert Arrow RecordBatch to telemetry records
    async fn arrow_batch_to_telemetry_records(&self, batch: &arrow::record_batch::RecordBatch) -> BridgeResult<Vec<TelemetryRecord>> {
        use arrow::array::{StringArray, Float64Array, Int64Array, TimestampNanosecondArray};
    ...
    fn avro_value_to_telemetry_record(&self, avro_value: avro_rs::types::Value) -> BridgeResult<TelemetryRecord> {
...
impl FileSource {
    ...
    async fn init_file_reader(&mut self) -> BridgeResult<()> {
        ...
        let file_reader = FileReader::new(&self.config.file_path, self.config.file_format.clone()).await?;
        ...
        if let Some(reader) = &self.file_reader {
            ...
            Ok(TelemetryBatch {
                ...
                metadata: {
                    ...
                    metadata.insert("file_format".to_string(), format!("{:?}", self.config.file_format));
                    metadata
                },
            })
        } else {
