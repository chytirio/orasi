...
impl SnowflakeWriter {
    ...
    pub async fn new(config: SnowflakeConfig) -> SnowflakeResult<Self> {
        info!(
            "Creating Snowflake writer for database: {}",
            config.database()
        );
...
impl LakehouseWriter for SnowflakeWriter {
    async fn write_metrics(&self, batch: MetricsBatch) -> BridgeResult<WriteResult> {
        debug!(
            "Writing metrics batch to Snowflake: {} records",
            batch.metrics.len()
        );
    ...
    async fn write_traces(&self, batch: TracesBatch) -> BridgeResult<WriteResult> {
        debug!(
            "Writing traces batch to Snowflake: {} records",
            batch.traces.len()
        );
        ...
        let duration = start_time.elapsed();
        info!(
            "Successfully wrote {} trace records to Snowflake",
            batch.traces.len()
        );
    ...
    async fn write_logs(&self, batch: LogsBatch) -> BridgeResult<WriteResult> {
        debug!(
            "Writing logs batch to Snowflake: {} records",
            batch.logs.len()
        );
        ...
        let duration = start_time.elapsed();
        info!(
            "Successfully wrote {} log records to Snowflake",
            batch.logs.len()
        );
    ...
    async fn write_batch(&self, batch: TelemetryBatch) -> BridgeResult<WriteResult> {
        debug!(
            "Writing telemetry batch to Snowflake: {} records",
            batch.records.len()
        );
        ...
        let duration = start_time.elapsed();
        info!(
            "Successfully wrote {} telemetry records to Snowflake",
            batch.records.len()
        );
