...
impl KafkaConsumer {
    ...
    async fn poll(&mut self) -> BridgeResult<Vec<(Option<Vec<u8>>, Vec<u8>)>> {
        let messages = self.consumer.poll()
            .map_err(|e| bridge_core::BridgeError::network(
                format!("Failed to poll messages from Kafka: {}", e)
            ))?;
        ...
        for message_set in messages.iter() {
            for message in message_set.messages() {
                let key = if message.key.is_empty() { None } else { Some(message.key.to_vec()) };
...
impl KafkaSource {
    ...
    async fn init_consumer(&mut self) -> BridgeResult<()> {
        info!("Initializing Kafka consumer for topic: {}", self.config.topic);
    ...
    async fn update_stats_from_result(&self, result: BridgeResult<TelemetryBatch>, value_len: usize) {
        match result {
            Ok(batch) => {
                // Update stats
    ...
    async fn process_kafka_message(&self, key: Option<Vec<u8>>, value: Vec<u8>) -> BridgeResult<TelemetryBatch> {
        ...
        let batch = match self.config.serialization_format {
            ...
            KafkaSerializationFormat::Json => {
                self.decode_json_message(&value).await?
            }
            KafkaSerializationFormat::Avro => {
                self.decode_avro_message(&value).await?
            }
            KafkaSerializationFormat::Protobuf => {
                self.decode_protobuf_message(&value).await?
            }
            KafkaSerializationFormat::Parquet => {
                self.decode_parquet_message(&value).await?
            }
        };
        ...
        if let Some(key) = key {
            metadata.insert("kafka_key".to_string(), String::from_utf8_lossy(&key).to_string());
        }
        metadata.insert("topic".to_string(), self.config.topic.clone());
        metadata.insert("group_id".to_string(), self.config.group_id.clone());
        metadata.insert("serialization_format".to_string(), format!("{:?}", self.config.serialization_format));
