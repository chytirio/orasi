...
impl FileWriter {
    ...
    async fn new(config: &FileSinkConfig) -> BridgeResult<Self> {
        ...
        if let Some(parent) = path.parent() {
            if !parent.exists() {
                tokio::fs::create_dir_all(parent).await.map_err(|e| {
                    bridge_core::BridgeError::stream(format!("Failed to create directory: {}", e))
                })?;
            }
        }
        ...
        let file = match config.write_mode {
            WriteMode::Append => OpenOptions::new()
                ...
                .map_err(|e| {
                    bridge_core::BridgeError::stream(format!(
            ...
            WriteMode::Rotate => {
                ...
                let rotated_path =
                    path.with_file_name(format!("{}_{}.{}", stem, timestamp, extension));
    ...
    async fn write_json(&mut self, batch: &TelemetryBatch) -> BridgeResult<()> {
        let json_data = serde_json::to_string_pretty(batch).map_err(|e| {
            bridge_core::BridgeError::stream(format!("Failed to serialize batch to JSON: {}", e))
        })?;
        ...
        self.file
            ...
            .await
            .map_err(|e| {
                bridge_core::BridgeError::stream(format!("Failed to write JSON to file: {}", e))
            })?;

        self.file.write_all(b"\n").await.map_err(|e| {
            bridge_core::BridgeError::stream(format!("Failed to write newline: {}", e))
        })?;
    ...
    async fn write_json_lines(&mut self, batch: &TelemetryBatch) -> BridgeResult<()> {
        for record in &batch.records {
            let json_line = serde_json::to_string(record).map_err(|e| {
                bridge_core::BridgeError::stream(format!(
                    "Failed to serialize record to JSON: {}",
                    e
                ))
            })?;
            ...
            self.file
                ...
                .await
                .map_err(|e| {
                    bridge_core::BridgeError::stream(format!("Failed to write JSON line: {}", e))
                })?;
    ...
    async fn write_csv(&mut self, batch: &TelemetryBatch) -> BridgeResult<()> {
        ...
        if self.records_written == 0 {
            ...
            self.file.write_all(header.as_bytes()).await.map_err(|e| {
                bridge_core::BridgeError::stream(format!("Failed to write CSV header: {}", e))
            })?;
        }
        ...
        for record in &batch.records {
            ...
            self.file
                ...
                .await
                .map_err(|e| {
                    bridge_core::BridgeError::stream(format!("Failed to write CSV line: {}", e))
                })?;
        }
    ...
    async fn flush(&mut self) -> BridgeResult<()> {
        self.file.flush().await.map_err(|e| {
            bridge_core::BridgeError::stream(format!("Failed to flush file: {}", e))
        })?;
        Ok(())
    }
...
mod tests {
    ...
    async fn test_file_sink_creation() {
        let config = FileSinkConfig::new("/tmp/test_file_sink.json".to_string(), FileFormat::Json);
    ...
    async fn test_file_sink_json_writing() {
        ...
        let config = FileSinkConfig::new(file_path.to_string_lossy().to_string(), FileFormat::Json);
    ...
    async fn test_file_sink_csv_writing() {
        ...
        let config = FileSinkConfig::new(file_path.to_string_lossy().to_string(), FileFormat::Csv);
